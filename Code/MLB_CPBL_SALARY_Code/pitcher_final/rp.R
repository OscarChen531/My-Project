###########Relief Pictcher######################
Relief_Pictcher_salary <- pitching_salary %>%
  filter(IPouts>0 & IPouts < 360)

## Descriptive Statistics
describe(batting_salary)
# dfSummary(Relief_Pictcher_salary)
# view(dfSummary(Relief_Pictcher_salary))

## Multivariable Linear Regression
## Multivariable Regression
model <- lm(salary ~ ., data = Relief_Pictcher_salary)
summary(model)
...
names(Relief_Pictcher_salary)
dim(Relief_Pictcher_salary)
sum(is.na(Relief_Pictcher_salary$salary))
batting_salary=na.omit(Relief_Pictcher_salary)
dim(Relief_Pictcher_salary)
sum(is.na(Relief_Pictcher_salary))
head(Relief_Pictcher_salary)
## PCR
library("pls")
set.seed (1000)
pcr.fit <- pcr(salary~W+L+G+SV+IPouts+H+ER+HR+BB+SO+BAOpp+IBB+WP+HBP+BK+BFP+GF+R+SH+SF+GIDP+ERA_plus+APR+ERA, 
               data = Relief_Pictcher_salary, scale = FALSE, validation = "CV")
summary(pcr.fit)
...
# Train vs CV
plot(pcr.fit, labels = c(1:dim(Relief_Pictcher_salary)[1]), which = "validation")
...
# Scores:
scoreplot(pcr.fit, labels = c(1:dim(Relief_Pictcher_salary)[1])) # only 1 and 2
scoreplot(pcr.fit, comps = 1:3, 
          labels = c(1:dim(Relief_Pictcher_salary)[1]))
...
library(caret)
set.seed(100)
preObj <- preProcess(Relief_Pictcher_salary, method=c("center", "scale"))
dd <- predict(preObj, Relief_Pictcher_salary)
dim(dd)
head(dd)
# dd = scale(Hitters)
...
pcr.fit = pcr(salary ~ W+L+G+SV+IPouts+H+ER+HR+BB+SO+BAOpp+IBB+WP+HBP+BK+BFP+GF+R+SH+SF+GIDP+ERA_plus+APR+ERA, data=train, scale=FALSE, 
...
pcr.fit.sc = pcr(salary ~ W+L+G+SV+IPouts+H+ER+HR+BB+SO+BAOpp+IBB+WP+HBP+BK+BFP+GF+R+SH+SF+GIDP+ERA_plus+APR+ERA, data=train, scale=TRUE, 
                 validation="CV", segments=10)
summary(pcr.fit.sc)
